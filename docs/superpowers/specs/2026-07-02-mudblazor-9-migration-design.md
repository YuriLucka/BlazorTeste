...
Em cada um: trocar apenas o *tipo* `MudDialogInstance` por `IMudDialogInstance` no campo `[CascadingParameter]`. Confirmado lendo o código-fonte real do MudBlazor nas tags `v8.15.0` e `v9.6.0` (`IMudDialogInstance.cs`): `.Close()`, `.Close(DialogResult)`, `.Cancel()` continuam **síncronos** — nenhuma mudança de assinatura nas chamadas `MudDialog.Close(...)`/`MudDialog.Cancel()` já existentes.
